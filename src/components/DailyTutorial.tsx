...
const DailyTutorial = ({ onClose }: { onClose: () => void }) => {
  const [currentSlide, setCurrentSlide] = useState(0);

  const nextSlide = () => {
    if (currentSlide < dailySlides.length - 1) {
      setCurrentSlide((prev) => prev + 1);
    } else {
      onClose();
      localStorage.setItem("wsprs-daily-tutorial-seen", "true");
    }
  };
  ...
  return (
    <div className="fixed inset-0 bg-black/70 backdrop-blur-md flex items-center justify-center z-50">
      ...
      >
        ...
        <img
          src={dailySlides[currentSlide].image}
          alt="Tutorial Step"
          className="rounded-lg shadow-lg w-full object-contain"
        />

        <p className="text-white text-lg opacity-90">
          {dailySlides[currentSlide].text}
        </p>
        ...
        >
          {currentSlide === dailySlides.length - 1 ? "Got It" : "Next"}
        </motion.button>
      </motion.div>
    </div>
  );
